...
//
// Based on https://github.com/simibac/ConfettiSwiftUI to render confetti effect on end of course
...
public struct ConfettiCannon: View {
    ...
    public init(counter:Binding<Int>,
        ...
        for confetti in confettis{
            for _ in colors{
                switch confetti {
                case .shape(_):
                    shapes.append(AnyView(confetti.view.frame(width: confettiSize, height: confettiSize, alignment: .center)))
                default:
                    shapes.append(AnyView(confetti.view.font(.system(size: confettiSize))))
                }
            }
        }
    ...
    public var body: some View {
        ...
        .onChange(of: counter) { newValue, oldValue in
            if firstAppear{
    ...
    var body: some View{
        ZStack{
            ForEach(0...confettiConfig.num-1, id:\.self){_ in
                ConfettiView(confettiConfig: confettiConfig, radius: radius)
            }
        }
        .onAppear(){
            if firstAppear{
                DispatchQueue.main.asyncAfter(deadline: .now() + confettiConfig.getAnimationDuration(radius: radius)) {
...
struct ConfettiView: View {
    ...
    func getAnimationDuration() -> CGFloat {
        return 0.2 + confettiConfig.getExplosionAnimationDuration(radius: radius) + getRandomExplosionTimeVariation()
    }
    ...
    func getDelayBeforeRainAnimation() -> TimeInterval {
        confettiConfig.getExplosionAnimationDuration(radius: radius) * 0.1
    }
    
    var body: some View{
        
        ConfettiAnimationView(shape:getShape(), color: confettiConfig.colors.randomElement()!, spinDirX: getSpinDirection(), spinDirZ: getSpinDirection())
            ...
            .onAppear(){
                withAnimation(getAnimation()) {
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + getDelayBeforeRainAnimation()) {
                        withAnimation(Animation.timingCurve(0.12, 0, 0.39, 0, duration: confettiConfig.getRainAnimationDuration(radius: radius))) {
...
class ConfettiConfig: ObservableObject {
    ...
    func getExplosionAnimationDuration(radius: CGFloat) -> Double {
#if os(watchOS)
        return 1.2
#else
        return Double(radius / 1300)
#endif
    }
    
    func getRainAnimationDuration(radius: CGFloat) -> Double {
        return Double((rainHeight + radius) / 200)
    }
    
    func getAnimationDuration(radius: CGFloat) -> Double {
        return getExplosionAnimationDuration(radius: radius) + getRainAnimationDuration(radius: radius)
    }
